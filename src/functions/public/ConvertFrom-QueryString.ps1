function ConvertFrom-QueryString {
    ...
    .EXAMPLE
        $kvCollection = 'taco=12&quesadilla=6&burrito=8' | ConvertFrom-QueryString
    ...
    .EXAMPLE
        $kvCollection = 'pagelen=50&state=OPEN&state=MERGED&q=created_on%3e%3d2024-01-25T16%3a37%3a56Z' | ConvertFrom-QueryString
        foreach($key in $kvCollection.Keys) {
            Write-Host "$key = $($kvCollection[$key])"
        }
